...
-   To use a project template, run `ptbk coder generate-boilerplates --template prompts/templates/backend.md`.
...
| Script | Purpose |
| --- | --- |
| `npm run coder:generate-boilerplates` | Runs `ptbk coder generate-boilerplates --template ./prompts/templates/common.md` to create new prompt files in `prompts/`. |
| `npm run coder:run` | Runs `ptbk coder run --harness openai-codex --model gpt-5.5 --thinking-level xhigh --agent agents/developer.book --context AGENTS.md` to execute the next ready prompt with shared repository context from `AGENTS.md`. |
| `npm run coder:verify` | Runs `ptbk coder verify` to archive verified prompts into `prompts/done/` and append repair prompts when needed. |
